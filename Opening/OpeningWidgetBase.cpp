...
void UOpeningWidgetBase::NativeConstruct()
{
	...
	UMediaPlayer* OpeningMediaPlayer = LoadObject<UMediaPlayer>(nullptr, TEXT("/Script/MediaAssets.MediaPlayer'/Game/Blueprints/Opening/Assets/OpeningMedia.OpeningMedia'"));

    if (OpeningMediaPlayer)
    {
        // 미디어 소스 로드
        //UMediaSource* MediaSource = LoadObject<UMediaSource>(nullptr, TEXT("/Script/MediaAssets.FileMediaSource'/Game/Movies/Opening.Opening'"));
        ...
        if (MediaSource && OpeningMediaPlayer->OpenSource(MediaSource))
        {
...
void UOpeningWidgetBase::OnMediaFinished()
{
	...
	case 2:
		...
		break;
	case 3:
		Temp = FString::Printf(TEXT("Z키를 이용해 상대방에게 정보를 알려줄 수 있습니다."));
		TipsTextBlock->SetText(FText::FromString(Temp));
		break;
	case 4:
