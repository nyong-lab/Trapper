...
void UJoinWidgetBase::OnOkButton()
{
	...
	// Pw
	if (FPassword.Len() < 8 || FPassword.Len() > 14)
	{
		FString Temp = FString::Printf(TEXT("8~14자 이내로 입력해야합니다."));
	...
	// Nickname
	if (FNickname.Len() < 2 || FNickname.Len() > 8)
	{
		FString Temp = FString::Printf(TEXT("닉네임은 최소 2글자, 최대 8글자까지 만들 수 있습니다."));
