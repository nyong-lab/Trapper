...
void UTrapperGameInstance::Init()
{
	...
	if (!bLoadSuccess)
	{
		...
		if (GameUserSettings && GEngine)
		{
			...
			DefaultDisplayModeString = "Window";
...
void UTrapperGameInstance::ConnectToGameServer()
{
	...
	FString IPAddress;
	if (FParse::Value(FCommandLine::Get(), TEXT("ServerIP="), IPAddress))
	{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("No Server IP Address provided in command line arguments."));
	}
