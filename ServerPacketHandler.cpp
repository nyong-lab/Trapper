		...
		if (UWorld* World = GWorld)
		{
			if (UGameInstance* GameInstance = World->GetGameInstance())
			{
				if (UTrapperGameInstance* MyGameInstance = Cast<UTrapperGameInstance>(GameInstance))
				{
					...
					// 친구 목록 가져오기
					Protocol::C_GET_FRIEND GetFriendsPkt;
					GetFriendsPkt.set_playerid(pkt.user().playerid());
					SendBufferRef SendBuffer1 = ServerPacketHandler::MakeSendBuffer(GetFriendsPkt);
					MyGameInstance->SendPacket(SendBuffer1);
					// 친구 요청 가져오기
					Protocol::C_GET_REQUESTS GetRequestPkt;
					GetRequestPkt.set_playerid(pkt.user().playerid());
					SendBufferRef SendBuffer2 = ServerPacketHandler::MakeSendBuffer(GetRequestPkt);
					MyGameInstance->SendPacket(SendBuffer2);
...
bool Handle_S_ADD_FRIEND(PacketSessionRef& session, Protocol::S_ADD_FRIEND& pkt)
{
	if (UWorld* World = GWorld)
	{
		if (UGameInstance* GameInstance = World->GetGameInstance())
		{
			if (UTrapperGameInstance* MyGameInstance = Cast<UTrapperGameInstance>(GameInstance))
			{
				TMap<FString, bool>& FriendRequests = MyGameInstance->GetFriends();
				FString FriendNickname = UTF8_TO_TCHAR(pkt.friend_().nickname().c_str());
				MyGameInstance->AddFriends(FriendNickname, pkt.friend_().isonline());
			}
		}
	}
