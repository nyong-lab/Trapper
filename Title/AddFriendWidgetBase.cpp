...
void UAddFriendWidgetBase::OnAddFriendStatusButton()
{
	...
	SendRequestpkt.set_playerid(std::string(TCHAR_TO_ANSI(*Cast<UTrapperGameInstance>(GetGameInstance())->GetPlayerID())));
	FString FFriend = AddFriendText->GetText().ToString();
	SendRequestpkt.set_friendnickname(std::string(TCHAR_TO_ANSI(*FFriend)));
