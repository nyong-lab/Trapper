...
void ATrapperPlayer::ServerRequiredCheat_Implementation(int32 Code)
{
...
void ATrapperPlayer::FClickStarted(const FInputActionValue& Value)
{
	...
	if (HasHit)
	{
		...
		if (!InteractActor)
		{
			//UE_LOG(LogQuest, Warning, TEXT("No Interact Actor"));
			return;
		}

		if (HasAuthority() && IsLocallyControlled())
		{
			//UE_LOG(LogQuest, Warning, TEXT("Interaction : Server"));
		...
		if (!HasAuthority() && IsLocallyControlled())
		{
			//UE_LOG(LogQuest, Warning, TEXT("Interaction : Client"));

			InteractActor->CheckActivate(30, false);
			ServerRPCPlayInstallAnim();
		}

		if (TrapperGameState->CurrentGameProgress == EGameProgress::Maintenance && TrapperGameState->MaintenanceCount >= 1)
		{
		...
		else if (TrapperGameState->CurrentGameProgress == EGameProgress::Tutorial)
		{
	...
	{
		//UE_LOG(LogQuest, Warning, TEXT("No Hit Actor"));
...
void ATrapperPlayer::GoMaintenance1(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(300);
	}
	else
	{
		ServerRequiredCheat(300);
	}
}
...
void ATrapperPlayer::GoMaintenance2(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(301);
	}
	else
	{
		ServerRequiredCheat(301);
	}
}
...
void ATrapperPlayer::GoMaintenance3(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(302);
	}
	else
	{
		ServerRequiredCheat(302);
	}
...
void ATrapperPlayer::GoMaintenance4(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(303);
	}
	else
	{
		ServerRequiredCheat(303);
	}
}
...
void ATrapperPlayer::GoMaintenance5(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(304);
	}
	else
	{
		ServerRequiredCheat(304);
	}
...
void ATrapperPlayer::GoMaintenance6(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(305);
	}
	else
	{
		ServerRequiredCheat(305);
	}
}
...
void ATrapperPlayer::AfterBoss(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(306);
	}
	else
	{
		ServerRequiredCheat(306);
	}
...
void ATrapperPlayer::GameRestart(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(307);
	}
	else
	{
		ServerRequiredCheat(307);
	}
}
...
void ATrapperPlayer::AddBone(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(308);
	}
	else
	{
		ServerRequiredCheat(308);
	}
...
void ATrapperPlayer::Invincibility(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(309);
	}
	else
	{
		ServerRequiredCheat(309);
	}
}
...
void ATrapperPlayer::MonsterKill(const FInputActionValue& Value)
{
	...
	if (HasAuthority())
	{
		TrapperGameState->OnEventExecute.Broadcast(310);
	}
	else
	{
		ServerRequiredCheat(310);
	}
}
...
float ATrapperPlayer::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	if (HasAuthority())
	{
		if (TrapperGameState->bIsInvincibilityMode)
		{
			return 0.f;
		}
...
{
	if (IsDead || bIsInvincibility) return 0.f;
...
void ATrapperPlayer::FindMagneticItem()
{
	...
	//DrawDebugSphere(GetWorld(), PlayerLocation, MagneticPullRange, 16, FColor::Blue, false, -1.f);
...
void ATrapperPlayer::AddBoneItem(int32 Count)
{
	...
	if (BoneItemBox < MaxItemCount)
	{
		BoneItemBox += Count;

		if (BoneItemBox > MaxItemCount)
			BoneItemBox = MaxItemCount;
	}

	ATrapperGameMode* MyGameMode = GetWorld()->GetAuthGameMode<ATrapperGameMode>();
	if (MyGameMode->GetCurrentGameProgress() == EGameProgress::Tutorial && Count == 1)
	{
		for (int i = 0; i < Count; i++)
		{
			GetWorld()->GetGameState<ATrapperGameState>()->OnQuestExecute.Broadcast(20, true);
		}
	}

	OnRep_BoneItemBox();

	//UE_LOG(LogTemp, Warning, TEXT("Server : Add Item / Total %d"), BoneItemBox);
}
...
void ATrapperPlayer::ServerRPCUseBoneItem_Implementation(int32 Count)
{
	UseBoneItem(Count);
}

void ATrapperPlayer::ServerRPCAddBoneItem_Implementation(int32 Count)
{
	AddBoneItem(Count);
}
...
void ATrapperPlayer::OnRep_BoneItemBox()
{
	if (PlayerController && IsLocallyControlled())
	{
		PlayerController->SetItemCount(BoneItemBox);
	}
}
...
{
	TrapMechanics->UpgradeCost(CostIcreaseDecreaseRate);
...
{
	if (GunpowderBarrelNum < MaxGunpowderBarrelNum)
	{
		return true;
	}
...
void ATrapperPlayer::ClientRPCUpdateTrapHudNum_Implementation(ETrapType Type, bool bInstall)
{
	...
	else if (Type == ETrapType::GunpowderBarrel)
	{
		...
		FString NumString = FString::Printf(TEXT("%d/5"), Num);
		PlayerController->TrapSelectHUDRef->BarrelText->SetText(FText::FromString(NumString));
	}
}
